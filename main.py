...
logging.basicConfig(format='%(asctime)s %(levelname)-8s %(message)s',
                    level=logging.DEBUG,
                    datefmt='%Y-%m-%d %H:%M:%S')
from main_process.video_process import run_video, run_video_multi_threaded
from main_process.socket_process import run_socket, run_socket_multi
...
if __name__ == '__main__':
    ...
    ap.add_argument("--mode", type=str, help="mode can be VIDEO_SINGLE/VIDEO_MULTI/SOCKET_SINGLE/SOCKET_MULTI", required=True)
    ...
    if args['mode'] in ['VIDEO_SINGLE', 'VIDEO_MULTI']:
        video_paths = []
